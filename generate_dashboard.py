...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .filter-section--search .chip-row:not(.theme-filter-row) .chip:not(.theme-chip):not(.chip--all):not(.active):not([aria-pressed="true"]):hover {
    border-color: color-mix(in srgb, var(--accent) 40%, var(--chip-accent-border));
    background: color-mix(in srgb, var(--chip-accent-bg) 90%, var(--accent) 10%);
    box-shadow: var(--shadow-sm);
    transform: none;
  }
  ...
  .filter-section--search .chip-row:not(.theme-filter-row) .chip:not(.theme-chip):not(.chip--all)[aria-pressed="true"] {
    ...
    font-weight: 400;
    color: var(--accent);
    background: color-mix(in srgb, var(--chip-accent-bg) 72%, var(--accent) 28%);
    border: 1px solid color-mix(in srgb, var(--accent) 55%, var(--chip-accent-border));
    padding: 6px 12px;
    border-radius: var(--radius-pill);
    box-shadow: var(--shadow-md);
    transform: none;
  }
  /* “Alles tonen”: zelfde typografie en padding als overige snelzoek-chips */
  .filter-section--search .chip-row:not(.theme-filter-row) .chip.chip--all,
